  ...
  'relative flex items-center justify-center rounded-full transition-colors cursor-pointer box-border transition-all min-w-fit disabled:pointer-events-none',
  {
    variants: {
      variant: {
        ...
        kakao: 'bg-[#FFE812]',
        shadow:
          'border border-white shadow-lg bg-white focus:border-primary transition',
      },
      size: {
...
export const Button = ({
  ...
  return (
    ...
    >
      {isSearch && (
        <figure className="absolute left-2 top-1/2 -translate-y-1/2 text-white bg-primary p-1 rounded-full">
          <Search size={16} />
        </figure>
      )}
      {children}
